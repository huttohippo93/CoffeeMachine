MENU = {
    "espresso": {
        "ingredients": {
            "water": 50,
            "coffee": 18,
        },
        "cost": 1.5,
    },
    "latte": {
        "ingredients": {
            "water": 200,
            "milk": 150,
            "coffee": 24,
        },
        "cost": 2.5,
    },
    "cappuccino": {
        "ingredients": {
            "water": 250,
            "milk": 100,
            "coffee": 24,
        },
        "cost": 3.0,
    }
}

profit = 0
resources = {
    "water": 300,
    "milk": 200,
    "coffee": 100,
}


def resource_check(order_ingredients):
    """Returns True when order can be made, returns False when ingredients are insufficient"""
    for item in order_ingredients:
        if order_ingredients[item] >= resources[item]:
            print(f"Sorry there is not enough {item}")
            return False
        return True


def process_coins():
    """Returns the total amount of coins inserted"""

    print("Please insert coins")
    total = int(input("How many quarters?: ")) * 0.25
    total += int(input("How many dimes?: ")) * 0.1
    total += int(input("How many nickles?: ")) * 0.05
    total += int(input("How many pennies?: ")) * 0.01

    return total


def is_trans_successful(money_received, drink_cost):
    """Return True when payment is accepted, or False if money is insufficient"""
    if money_received >= drink_cost:
        change = round(money_received - drink_cost, 2)
        print(f"Here is ${change} in change")
        global profit
        profit += drink_cost
        return True
    else:
        print("Sorry that's not enough money. Money refunded.")
        return False


def make_coffee(drink_name, order_ingredients):
    """Subtract the required ingredients from the resources"""
    for item in order_ingredients:
        resources[item] -= order_ingredients[item]
    print(f"Here is your {drink_name}. Enjoy!")

# User Prompt
coffeeMakerOn = True

while coffeeMakerOn:
    userInput = input("What would you like? (espresso/latte/cappuccino) ")
    # this print statement is for testing only remove/comment out when finished
    # print(userInput)

    # Decide what user input

    if userInput == "espresso":
        # for testing
        # print(userInput)
        drink = MENU[userInput]
        if resource_check(drink["ingredients"]):
            payment = process_coins()
            if is_trans_successful(payment, drink["cost"]):
                make_coffee(userInput, drink["ingredients"])
    elif userInput == "latte":
        # for testing
        # print(userInput)
        drink = MENU[userInput]
        if resource_check(drink["ingredients"]):
            payment = process_coins()
            if is_trans_successful(payment, drink["cost"]):
                make_coffee(userInput, drink["ingredients"])
    elif userInput == "cappuccino":
        # for testing
        # print(userInput)
        drink = MENU[userInput]
        if resource_check(drink["ingredients"]):
            payment = process_coins()
            if is_trans_successful(payment, drink["cost"]):
                make_coffee(userInput, drink["ingredients"])
    elif userInput == "off":
        print("Shutting down")
        coffeeMakerOn = False
        break
    elif userInput == "report":
        print(f"Milk: {resources['milk']}ml")
        print(f"Water: {resources['water']}ml")
        print(f"Coffee: {resources['coffee']}g")
        print(f"Money: ${profit}")
    else:
        print("You selected an incorrect item")
